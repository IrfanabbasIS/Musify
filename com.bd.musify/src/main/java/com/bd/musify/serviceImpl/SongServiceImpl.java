...
public class SongServiceImpl implements SongService {
    ...
    public Object getAllSongs(Long userId, int page, int size, String search) {
        ...
        if (hasUserid && hasSearch) {
            songPage = songRepository
                    .findByAppUserIdAndTitleContainingIgnoreCaseOrAppUserIdAndArtistContainingIgnoreCase(
                            userId, search.trim(), userId, search.trim(), null);
        } else if (hasSearch) {
            songPage = songRepository.findByTitleContainingIgnoreCaseOrArtistContainingIgnoreCase(search.trim(),
    ...
    // fileHandlerUtil.getFileExtension(songFile.getOriginalFilename());
    ...
    // CLOUDINARY UPLOADS METHODS
    private String processSongFile(MultipartFile songFile, String uniqueid) {

        return cloudinaryService.uploadSong(songFile, uniqueid);
    }

    // UPLOADS IMAGE IN LOCAL METHOS
    ...
    private String processImageFile(MultipartFile imagFile, String uniqueid) {
        ...
        return cloudinaryService.uploadImage(imagFile, uniqueid);
    }
    ...
    // NEW cloudinary method
    private void deleteOldSongFile(String songUrl) {
        if (songUrl != null && songUrl.contains("res.cloudinary.com")) {
            String publicId = extractCloudinaryPublicId(songUrl);
            if (publicId != null) {
    ...
    // helper method for cloudinary only
    private String extractCloudinaryPublicId(String url) {
        try {
            ...
            String publicIdWithExtension = url.substring(uploadIndex + uploadMarker.length());

            // Remove Cloudinary transformation/version information if present
            ...
            if (dotIndex != -1) {
                publicIdWithExtension = publicIdWithExtension.substring(0, dotIndex);
            }
